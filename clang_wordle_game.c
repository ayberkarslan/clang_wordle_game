...
void checkWord(int wordNum) {
    ...
    if (strcmp(inputWord, words[wordNum]) == 0) {
        //printf("Tebrikler! %s kelimesini dogru bildiniz!\n", words[wordNum]);
        //bunu sildim zaten ana while içinde if koyduk
            ...
            if (inputWord[i] == words[wordNum][i]) {
              //  printf("%c", inputWord[i]); // ya da words[wordNum][i]
                ...
                if (strchr(words[wordNum], inputWord[i]) != NULL) { //yeri yanlış olanlar
                    ...
                    if (strchr(wrongButExist, inputWord[i]) == NULL) { // Eğer listede yoksa
                    ...
                    // Olmayan harf için:
                    if (strchr(nonExist, inputWord[i]) == NULL) { // Eğer listede yoksa
...
int main(void) {
    ...
    for (int n=0; n<10 ; n++) {
        ...
        while (1){
            ...
            if (strcmp(inputWord, words[n]) == 0) { // eğer kelime doğruysa status basmayıp direkt döngüden çıktık
    ...
    float success= (10.0/totalTries)*100; //başarı oranı da hesapladık yüzde olarak - 10.0 yazdık çünkü tam sayı/tam sayı olursa gcc derleyicisi virgulden sonrasını atar
    printf(" Basari yuzdeniz       : %.1f                         \n", success); // %% kullanarak yüzdeyi, .1f ile küsuratı ayarladık
    printf("====================================================\n");
    printf("\033[0m\n");
